#ifndef ___SRC_CORE_ECS_ARCHETYPE_H___
#define ___SRC_CORE_ECS_ARCHETYPE_H___
	...
	*/
	...
	public:
		...
		*/
		 ...
		 */
		const bool IsContain(const Archetype& _other) const noexcept
		{
			std::lock_guard<std::mutex> lock(m_Mutex);
		 ...
		 */
		template <typename CompT>
		const std::size_t GetMemoryOffset()
		{
			std::lock_guard<std::mutex> lock(m_Mutex);
			std::size_t result = 0;
			std::size_t compId = TypeManager::TypeInfo<CompT>::GetID();
		 ...
		 * @return std::size_t 指定されたコンポーネントのメモリオフセット。
		 */
		const std::size_t GetMemroyOffsetByIndex(std::size_t _index)
		{
			std::lock_guard<std::mutex> lock(m_Mutex);
		 ...
		 */
		const std::size_t GetMemorySizeByIndex(std::size_t _index)
		{
			std::lock_guard<std::mutex> lock(m_Mutex);
		 ...
		 */
		const std::size_t GetComponentIdByIndex(std::size_t _index)
		{
			std::lock_guard<std::mutex> lock(m_Mutex);
		...
		*/
		const std::size_t GetArchetypeMemorySize() const noexcept
		{
			std::lock_guard<std::mutex> lock(m_Mutex);
		...
		*/
		const std::size_t GetArchetypeSize() const noexcept
		{
			std::lock_guard<std::mutex> lock(m_Mutex);
		...
		* @return アーキタイプのシグネチャ。
		*/
		const std::bitset<utils::cMaxComponentSize> GetSignature() const
		{
			return m_Signature;
		}
...
#endif //!___SRC_CORE_ECS_ARCHETYPE_H___