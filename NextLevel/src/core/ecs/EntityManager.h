#ifndef ___SRC_CORE_ECS_ENTITYMANAGER_H___
#define ___SRC_CORE_ECS_ENTITYMANAGER_H___
...
namespace ecs
{
	...
	*/
	...
	public:
		...
		*/
		const bool ExistEntity(const Entity& _entity)
		{
			const std::uint32_t entityIndex = utils::GetIndex(_entity.m_Idntifier);
		...
		*/
		template <typename CompT>
		void SetComponent(const Entity& _entity, const CompT& _data)
		{
			if (!ExistEntity(_entity)) return;
		...
		*/
		template <typename CompT>
		CompT* GetComponent(const Entity& _entity)
		{
		...
		*/
		std::vector<Chunk*> GetContainChunkList(const Archetype& _archetype) const
		{
		...
		*/
		const std::uint32_t GetAndCreateChunkIndex(const Archetype& _archetype, bool _maxCheck = false) const
		{
	...
	private:
		...
		*/
		Entity CreateEntity(const Archetype& _archetype)
		{
...
#endif //!___SRC_CORE_ECS_ENTITYMANAGER_H___